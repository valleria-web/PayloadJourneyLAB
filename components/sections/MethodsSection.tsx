  ...
  return (
    <Section id="metodos" className="border-y border-line bg-subtle">
      <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
        ...
        <div className="w-fit rounded-full border border-line bg-white px-4 py-2 font-mono text-xs text-accent">
        ...
        {siteContent.methods.items.map((method, index) => (
          ...
          >
            <div>
              ...
              {method.category ? (
                <p className="mb-3 font-mono text-xs font-semibold uppercase tracking-[0.08em] text-accent">
                  {method.category}
                </p>
              ) : null}
              ...
              {method.status ? (
                <p className="mt-4 font-mono text-xs font-semibold text-accent">{method.status}</p>
              ) : null}
            </div>
            <ArrowUpRight
              aria-hidden="true"
      ...
      <p className="mt-8 rounded-xl border border-line bg-white/80 p-6 text-sm leading-6 text-graphite">
        <span className="font-mono text-accent">{">_"}</span> {siteContent.methods.relation}
      </p>
    </Section>
  );
}
